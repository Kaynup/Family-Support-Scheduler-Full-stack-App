...
- SQL layer: [Querying to the database with Python](../backend-docs/sql_queries.md)
- Database Tests: [Testing the SQL layer scripts](../testing-docs/backend_testing.md)

> `bills` table stores a single bill record per row. The schema is made around one bill having a title, an amount, due date and creation date, a status, and an optional grouping label called category.
