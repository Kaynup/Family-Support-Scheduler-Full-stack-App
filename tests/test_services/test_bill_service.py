...
from app.constants import STATUS_UNPAID, STATUS_PAID, INTERVAL_NONE
...
def test_create_bill(mock_insert_bill):
    ...
    out = create_bill(
        ...
        total_amount=100.0, status=STATUS_UNPAID, category="test", recurring_interval=INTERVAL_NONE
    )
...
def test_list_bills(mock_select_all):
    mock_select_all.return_value = [
        (1, "test", date.today(), date.today(), 100.0, STATUS_UNPAID, "test", INTERVAL_NONE, "N")
    ]
    out = list_bills()
    assert out["OK"] is True
...
def test_mark_bill_status(mock_select_bill, mock_update):
    mock_select_bill.return_value = (1, "test", date.today(), date.today(), 100.0, STATUS_UNPAID, "test", INTERVAL_NONE, "N")
...
def test_list_bills_upcoming(mock_select_upcoming):
    mock_select_upcoming.return_value = [
        (1, "test2", date.today(), date.today() + timedelta(days=2), 100.0, STATUS_UNPAID, "test", INTERVAL_NONE, "N"),
        (2, "test3", date.today(), date.today() + timedelta(days=3), 100.0, STATUS_UNPAID, "test", INTERVAL_NONE, "N"),
    ]
...
def test_search_bills_by_name(mock_select_by_name):
    mock_select_by_name.return_value = [
        (1, "search-test", date.today(), date.today(), 100.0, STATUS_UNPAID, "test", INTERVAL_NONE, "N")
    ]
