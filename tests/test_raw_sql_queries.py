...
# Utils
def _create_test_bill(category="test"):
    name = f"pytest-sql-{time.time_ns()}"
...
def test_insert_bill():
    bill_id = _create_test_bill(category="insert")
    try:
...
def test_select_num_day_dues():
    bill_id = _create_test_bill(category="num_day_due")
    try:
...
def test_update_bill_status():
    bill_id = _create_test_bill(category="update")
    try:
        assert any(r[0] == bill_id for r in select_num_day_dues(3))
...
def test_delete_bill_by_id():
    bill_id = _create_test_bill(category="delete")
    try:
