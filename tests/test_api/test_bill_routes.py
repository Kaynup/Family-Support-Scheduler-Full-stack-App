...
app.dependency_overrides[app.dependency_overrides.get("require_beneficiary_role", lambda: None)] = override_require_beneficiary_role

@patch('app.routes.bill_routes.create_bill')
def test_create_bill(mock_create_bill):
    mock_create_bill.return_value = {"OK": True, "data": {"id": 1}, "message": "Success"}
    payload = {
        "name": "test-api",
        "due_date": "2099-05-01",
...
def test_delete_bill(mock_delete_bill):
    mock_delete_bill.return_value = {"OK": True, "data": {"id": 1}, "message": "Deleted"}
...
def test_api_search_bills(mock_search_bills):
    mock_search_bills.return_value = {"OK": True, "data": [{"id": 1, "name": "pytest-api"}], "message": "Found", "total_count": 1}
