...
def _extract_bill_payment_fields(bill_row):
    ...
    if len(bill_row) > 6 and bill_row[2] in (STATUS_UNPAID, STATUS_PAID):
        return (
            bill_row[2],
            float(bill_row[6]),
            bill_row[3] if len(bill_row) > 3 else None,
        )
...
def pay_bill_via_remittance(
    bill_id,
    sender_user_id,
    amount,
    currency=CURRENCY_USDT,
    payment_method=PAYMENT_METHOD_STABLECOIN,
):
    """
    ...
    if bill_status != STATUS_UNPAID:
        raise RemittanceValidationError(
            f"Bill {bill_id} is already {bill_status} and cannot be paid again."
        )
